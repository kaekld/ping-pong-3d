...
func _physics_process(delta):
	...
	var move_right = Input.is_action_pressed("RightP1")
	var move_left = Input.is_action_pressed("LeftP1")
	...
	if not is_dashing:
		if Input.is_action_just_pressed("DashP1") and move_right:
			is_dashing = true
			dash_time = dash_duration
			dash_direction = 1
		elif Input.is_action_just_pressed("DashP1") and move_left:
			is_dashing = true
			dash_time = dash_duration
			dash_direction = -1
